...
def bot_init(event_loop, token):
    ...
    async def generation_captha(message):
        ...
        image.write(captcha_text, f'/home/str/5arc/img/{message.from_user.id}.png')
        with open(f'/home/str/5arc/img/{message.from_user.id}.png', 'rb') as photo:
    ...
    def generation_keyboard():
        ...
        btn_operator = InlineKeyboardButton(text="🎒Прайс🎒", url="https://telegra.ph/Arktik-SHOP-12-15")
        btn_support = InlineKeyboardButton(text="🔥Оператор🔥", url="https://t.me/+14305884728")
        btn_4 = InlineKeyboardButton(text="🔥Саппорт🔥", url="https://t.me/+15183352353")
        btn_3 = InlineKeyboardButton(text="💰ТРУДОУСТРОЙСТВО💰", url="https://t.me/Mr_NORKA")
        btn_2 = InlineKeyboardButton(text="ДОКТОР", url="https://t.me/doggok45")
        btn_1 = InlineKeyboardButton(text="🤖Инструкция по созданию рферального бота", url="https://telegra.ph/Instrukiya-referalno-sistemy-04-08-2")
        return keyboard.add(btn_balance).add(btn_my_bot).add(btn_ref).add(btn_last_order).add(btn_operator).add(
            btn_support).add(btn_4).add(btn_3).add(btn_2).add(btn_1)
